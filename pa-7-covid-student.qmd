...
state_totals |>
  ...
  geom_col(aes(x = total,
               y = fct_reorder(state, total),
               fill = fct_reorder(state, total))) +
  labs(x = "total COVID cases",
       y = "State in decending order") +
...
>No because we should look at a porportion of the population
...
```{r}
state_totals |> 
  left_join(us_pop, by = c("state" = "NAME")) |> 
  ...
  ggplot() +
  geom_col(aes(x = prop,
               y = fct_reorder(state, prop),
               fill = fct_reorder(state, prop))) +
  labs(x = "Proportion COVID cases among total population",
       y = "State in decending order") +
...
> People could have gotten multiple cases of covid and been counted twice. Also, there are different lengths of time that each case of covid lasts. We are assuming that population did not change.
...
> What months had the most new cases of COVID? This would not require any additional information.
